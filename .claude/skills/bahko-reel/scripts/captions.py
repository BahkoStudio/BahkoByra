...
def _find_font():
    ...
    cands = [
        # Outfit först: brand.json säger Outfit rakt igenom för Bahko-material.
        os.path.join(here, "..", "assets", "Outfit[wght].ttf"),
    ...
    for c in cands:
        if os.path.exists(c):
            return c
    raise SystemExit("Ingen textfont hittad. Sätt CAPTION_FONT=/sökväg/till/font.ttf "
                     "eller behåll assets/Outfit[wght].ttf i skillen.")
FONT = _find_font()
