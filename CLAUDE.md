...
| puzzle-me | `/puzzle-me [bygge + trade/ort]` | "här är ett bygge", "dirigera bygget", "nischa bygget", "anpassa mallen till nischen", "ge mig reel-script för bygget", "hur ska vi filma det här" |
...
- **puzzle-me** — Tar ett **inkommande bygge** (mall, template, sida någon lämnat över), dirigerar om det mot bygg/hantverksnischens smärta, och lämnar sedan ett **reel-paket** (manus + inspelningsinstruktioner + caption) för att visa upp jobbet på @bahkostudio. Kärnan är **de två smärtlagren**: hantverkaren (vår köpare) styr reelen, villaägaren som ska handla upp jobbet styr all copy i bygget — blandas de ihop får kunden en sajt som pratar med sig själv. Nisch-kunskap (rädsla-till-element-checklista, per-trade-ord, reel-vinklar) i `.claude/skills/puzzle-me/nisch.md`. Reel-format låst: svenska, 20–30s, 9:16, skärminspelning + voiceover, sekundsatt beat-schema, 50–75 ord. Kostar **noll credits** — genererar aldrig bild/video. Skiljer sig från syskonen: `scroll-cinematic` bygger nytt från grunden, `demo-recopy` byter bara copy på en befintlig demo, `instagram-engine` gör veckobatchar utan koppling till ett specifikt bygge. Output: dirigerat bygge + `content/reels/bygge-[kund].md`.
