...
- **famous-reel-editor** — Klipper ett rått talking-head-klipp (vertikalt eller 4K liggande) till färdig 9:16-reel: talande huvud i nedre bandet, animerade motion-graphics-scheman i övre, karaoke-undertexter (ett ord), musik och 1-2 AI-B-roll via Higgsfield. **Bahko-läge (2026-08-18):** `scripts/bahko_assets.py` hämtar palett/märke/maskot/Outfit ur `brand.json` vid varje körning (inget varumärkesmaterial dupliceras in i skillen), `gen_bahko.py` bygger kortlagret, `maskot_frames.py` renderar maskoten som sömlös PNG-loop och `compose_bahko.sh` komponerar. **Maskoten står vid Mathias i de fönster där hon passar** (`MASKOT_FONSTER`), intron är avskalad och outron bär varumärkeslåset (logo-dark.svg — logo.svg är marinblå och försvinner på mörkt band). `brand.json` uppdaterad 2026-08-18, källa och praxis stämmer. GSAP paketeras lokalt: från CDN renderas korten helt utan animation om nätet är stängt. Regler och mätta värden: `.claude/skills/famous-reel-editor/references/bahko-brand.md`. **Saknas:** `assets/bg-music.m4a` (ange `MUSIC=/sökväg`), `assets/logos/openai.svg` (borttagen ur simple-icons) och `SETUP.md`. Körs lokalt — ffmpeg, ffprobe och Pillow krävs. Kör aldrig Higgsfield-credits utan beställning.
