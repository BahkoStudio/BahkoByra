...
| bahko-reel | `/bahko-reel` | "bahko-reel", "reel med grafik över mig", "reel för @bahkostudio", "gör en reel av det här klippet" |
...
- **famous-reel-editor** — Författarens skill (Ambra-stilen): delad ram, grafik i övre 864 px och talande huvud i nedre 1056. Karaoke-undertexter, musik och 1-2 AI-B-roll via Higgsfield. Behållen som originalet. **Två äkta buggfixar gjorda 2026-08-18:** `loudnorm` gav 96 kHz medan sampelantalet svarade mot 48 kHz, vilket gjorde ljudet dubbelt så snabbt (fixat med `aresample=48000` + `-ar 48000`), och andra passet saknade `-shortest` (100 ms drift). **Saknas:** `assets/bg-music.m4a`, `assets/logos/openai.svg` (borttagen ur simple-icons) och `SETUP.md`. Körs lokalt — ffmpeg, ffprobe och Pillow krävs.
- **bahko-reel** — Bahko-reels där Mathias pratar: **videon fyller HELA 9:16 hela tiden** och grafiken ligger som ett alfa-lager ovanpå (beslut 2026-08-18, ersätter den delade ramen). Grafiken MÅSTE renderas med `--format mov` — en mp4 saknar alfakanal och täcker videon svart; `compose.sh` kontrollerar pixelformatet och vägrar köra annars. Panelzon y 210-910 för att undvika ansiktet, Instagrams knapprad (högerkant) och bildtexten (nedtill). **Text, musik, SFX och maskot är val Mathias styr** — inget läggs på av sig självt. SFX syntetiseras lokalt utan credits; musik gör den INTE (lägg ett spår på `assets/musik.m4a`). `brand.py` läser palett, logga och maskotregler ur `brand.json` vid varje körning. Klippstegen lånas ur famous-reel-editor, inte dubblerade. Regler: `.claude/skills/bahko-reel/references/brand.md`. Kör aldrig Higgsfield-credits utan beställning.
